      ...
      if (cards) {
        gsap.fromTo(
          ...
          { y: 60, opacity: 0 },
          {
            ...
            scrollTrigger: { trigger: gridRef.current, start: "top 85%", ...trigger },
          }
        );
      }
    }, sectionRef);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {AGENTS.map((agent) => (
            <div
              key={agent.id}
              className="agent-card bg-[radial-gradient(114.09%_141.5%_at_100%_0%,rgba(30,64,175,0)_0%,rgba(30,64,175,0.05)_100%)] h-[420px] rounded-2xl p-6 text-center hover:shadow-md transition-shadow"
              style={{ opacity: 0 }}
            >
              {/* Avatar */}
              <Image
           ...
           shadow-[0_10px_15px_-3px_rgba(0,0,0,0.10),0_4px_6px_-4px_rgba(0,0,0,0.10)]"
              ...
              <Link
                ...
                className="inline-block w-full bg-primary-600 mt-[24px] hover:bg-primary-700 text-white text-sm font-semibold py-2.5 rounded-full transition-colors text-center"
              >
                View Profile
              </Link>
            </div>
