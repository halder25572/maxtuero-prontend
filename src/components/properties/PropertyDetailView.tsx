...
function SimilarPropertyCard({ property }: { property: Property }) {
  return (
    ...
    >
      ...
      <div className="p-4">
        <p className="text-primary-600 font-bold text-lg">{formatPrice(property.price)}</p>
        <h3 className="mt-1 text-sm font-semibold text-gray-900 line-clamp-2">{property.title}</h3>
...
export default function PropertyDetailView({ property }: PropertyDetailProps) {
  const encodedLocation = encodeURIComponent(`${property.location}, Dominican Republic`);

  const galleryImages = useMemo(() => {
    const fallbackImages = FEATURED_PROPERTIES.filter((item) => item.id !== property.id)
  ...
  const similarProperties = FEATURED_PROPERTIES.filter((item) => item.id !== property.id).slice(0, 3);
  ...
  return (
    <div className=" bg-white">
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div
                ...
                onMouseMove={(event) => {
                  const now = Date.now();
                  if (now - lastMouseTriggerRef.current < 650) return;
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4 bg-[#F8FAFC] border border-[#E5E7EB] py-[33px] px-[94px] rounded-[16px]">
              {stats.map((item) => {
            ...
            <section>
              ...
              <div className="mt-4 space-y-4 text-sm leading-7 text-gray-600">
                ...
                <p>
                  This property combines a refined layout with practical living spaces, designed for
                  buyers who want a polished coastal lifestyle and strong long-term value.
                </p>
                <p>
                  Features include private outdoor areas, secure access, and a location close to key
          ...
          <aside className="space-y-6">
            ...
            <section className="rounded-3xl bg-white border border-gray-100 shadow-sm p-6">
              <div className="flex items-center gap-2 text-primary-600 mb-3">
                <Sparkles size={16} />
                <p className="text-xs font-semibold uppercase tracking-[0.2em]">Why It Stands Out</p>
              </div>
              <p className="text-sm leading-6 text-gray-600">
                Premium finishes, prime location, and a streamlined contact flow make this property
                feel aligned with the rest of the polished platform.
              </p>
      ...
      <section className="w-full bg-[#F9FAFB] shadow-sm pt-6 pb-20 px-4 sm:px-6 lg:px-8 mt-[100px]">
